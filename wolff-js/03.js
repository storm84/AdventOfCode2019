...
for (let w=0;w<wires.length;w++){
	...
	for (const move of wire) {
		const dir=move[0]
		const dist=+move.substr(1)
		const moveX=dir==='R'||dir==='L'
		const amount=(dir==='R'||dir==='U')?1:-1
		for (let i=0;i<dist;i++){
			...
			if (map[key] === undefined) {
				...
				map[key] = 'X'
				const curDist = Math.abs(x)+Math.abs(y)
				if (curDist < minIntersection) {
					minIntersection=curDist
				}

				const curSteps = map2[key] + wDist
