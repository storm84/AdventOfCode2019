...
function calc(input) {
  const events = input
    ...
    .sort()
    .map(x => {
      const match = /(.+?):(\d\d)] (Guard #(\d+)|f|w)/.exec(x);
      const date = match[1];
      const minute = +match[2];
      const event = match[3];
      const guard = +match[4];
      return { date, minute, event: event[0], guard };
    });

  const sleepByGuard = {};
  let lastGuard, lastSleep;
  for (const { date, minute, event, guard } of events) {
    if (event === "G") {
      // new guard
      lastGuard = guard;
      lastSleep = undefined;
      if (!sleepByGuard[guard]) {
        sleepByGuard[guard] = {
          guard: guard,
          totMinutes: 0,
          byMinute: new Array(60).fill(0),
        };
      }
    } else if (event === "f") {
      // fall asleep
      ...
      // wake up
      sleepByGuard[lastGuard].totMinutes += minute - lastSleep;
      for (let i = lastSleep; i < minute; i++) {
        sleepByGuard[lastGuard].byMinute[i]++;
      }
      lastSleep = undefined;
    }
  }

  const guards = Object.values(sleepByGuard);

  const sleepy = guards.sort((a, b) => b.totMinutes - a.totMinutes)[0];

  function getMaxSleep(sleepy) {
    let maxSleep = 0;
    let maxSleepMinute = -1;
    sleepy.byMinute.forEach((x, m) => {
      if (x > maxSleep) {
        maxSleep = x;
        maxSleepMinute = m;
      }
    ...
    return { maxSleep, maxSleepMinute, guard: sleepy };
  }

  // // part 1
  // const maxSleepMinute = getMaxSleep(sleepy).maxSleepMinute;
  // return { sleepy, maxSleepMinute, part1: sleepy.guard * maxSleepMinute };

  // part 2
  let maxSleep = 0;
  let max;
  guards.map(getMaxSleep).forEach(x => {
    if (x.maxSleep > maxSleep) {
      maxSleep = x.maxSleep;
      max = x;
    }
  });

  return { maxSleep, max, part2: max.maxSleepMinute * max.guard.guard };
}
